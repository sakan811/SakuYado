...
declare global {
  namespace Cypress {
    interface Chainable {
      addHotel(name: string, price: number, rating: number, currency?: string): Chainable<Element>
    }
  }
}

Cypress.Commands.add('addHotel', (name: string, price: number, rating: number, currency = 'USD') => {
  cy.visit('/hotels/add')
